...
Callable traits, concepts, and utilities for the ndof framework family
...
```
requires = "ndof-callable/0.1.0"
```

Or as a plain CMake package (no package manager required):

```cmake
find_package(ndof-callable REQUIRED)
target_link_libraries(your_target PRIVATE ndof::callable)
```
